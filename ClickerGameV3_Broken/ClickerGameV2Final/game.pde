...
void game() {
  ...
  // millis() % (abs(difficultySlider - 21) * 1000 + 20) > (abs(difficultySlider - 21) * 1000)
  ...
  for (int index = 0; index < 10; index++) {
    if (secretcode[index] == currentcode[index]) {
      codeIsTrue++;
    }
  }
  ...
  if (targetCount == 0 || codeIsTrue == 10) mode = GAMEWON;
}
